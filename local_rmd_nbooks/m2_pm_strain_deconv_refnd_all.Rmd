...
```{r, fig.height=4, fig.width=20, fig.show='hide', results='hide', warning=FALSE, message=FALSE}
##Function to plot snps per the number of SNPs falling in the different filter categories for each stage + strain cluster
...
```{r, results='asis', fig.height=6, fig.width=20}
...
### Heatmap of IBS metrics between pairwise strain comparisons {.tabset} 
...
gtypes_chr_cln_ibs_mtx <- function(gtypes_chr_pos){
  ...
  gdsSub = snpgdsLDpruning(gds, ld.threshold = 0.2, verbose = F)
  
  ibs = snpgdsIBS(gds, num.thread = 4, verbose = F)
...
### Visualize identical and different SNPs between any pairwise strain comparisons {.tabset} 

```{r}
## Function for ploting and counting identical and different SNPs between any pairwise strain comparisons
...
### Visualize simpler chromosome plots(no continous gradient)  {.tabset} 
...
### Visualize smoother chromosome plots (with continous gradient)  {.tabset} 
...
```{r, warning=FALSE, eval=T}
## Save for trial notebook
saveRDS(ss_afm_chromp_smth, paste0("data/processed/",species, "/ss_afm_chromp_smth.RDS"))
```
...
```{r, warning=FALSE, eval=T,  fig.show='hide'}
...
### Manually checking the accuracy of K and assigning most accurate K estimate 
...
```{r, fig.height=18, fig.width=18, results='hide'}
